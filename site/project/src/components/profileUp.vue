...
export default{
    ...
    data() {
        return {
            ...
            sexOld:'',
            itog:false,
            inputFocus:[false,false,false,false,false]
        }
    },
    mounted() {
        ...
        .then(response => { 
            ...
            this.lastName=this.user[0].name_last;
            this.sex=this.user[0].sex
            this.day=getDay(this.user[0].date);

            this.firstNameOld=this.user[0].name;
            this.lastNameOld=this.user[0].name_last;
            this.sexOld=this.user[0].sex
    ...
    methods:{
        ...
        Update(){
            ...
            if(this.firstName.trim()!='' && this.lastName.trim()!='' && this.day!='' && this.month!='' 
            && this.year!='' && this.sex!=''){
            const userData = {
                    name: this.firstName,
                ...
                axios.post('http://cp/site/project/php/profileUpdate.php', userData, {
                timeout: 50000,
                headers: {
                    'Content-Type': 'application/json',
                    'Content-Type': 'application/x-www-form-urlencoded'
...
<template>
    ...
    <div class="field">
        ...
        <form @click.prevent>
            ...
            <div class="radio">   
                <label for="one" @click="this.sex=1" class="radio-cursor">
                    <input type="radio" id="one" value="1" v-model="sex" >
                    <span>Мужской</span>
                </label>
                <label for="two" @click="this.sex=2" class="radio-cursor">
                    <input type="radio" id="two" value="2" v-model="sex" >
                    <span>Женский</span>
                </label>
            </div>
            <button @click.stop.prevent="Update">Сохранить изменения</button>
        </form>    
    </div>
